...
export default function SurveillanceInterface() {
  ...
  const { socket, isConnected, emit } = useSocket({
    autoConnect: true,
    onConnect: () => {
      console.log("Connected to Third Eye server");
      toast({
        title: "Third Eye Connected",
        description: "Successfully connected to monitoring server.",
        variant: "success",
      });
    },
    onDisconnect: () => {
      console.log("Disconnected from Third Eye server");
    ...
    onError: (error) => {
      ...
      toast({
        ...
        description: "Failed to connect to Third Eye server.",
        variant: "destructive",
      });
    },
  });
  ...
  const toggleFrameStreaming = () => {
    if (isStreamingFrames) {
      ...
      setIsStreamingFrames(false);
      toast({
        title: "Frame Streaming Stopped",
        description: "Video frames are no longer being sent for analysis.",
        variant: "default",
      });
    } else {
      // Start streaming frames every 200ms (5 FPS)
      frameIntervalRef.current = setInterval(captureAndSendFrame, 200);
      setIsStreamingFrames(true);
      toast({
        title: "Frame Streaming Started",
        description: "Video frames are being sent for AI analysis.",
        variant: "success",
      });
    }
  };
  ...
  useEffect(() => {
    const startCamera = async () => {
      try {
        ...
        if (videoRef.current) {
          ...
          // Start frame streaming once video is loaded and playing
          videoRef.current.onloadedmetadata = () => {
            if (isConnected && !isStreamingFrames) {
              setTimeout(() => {
                toggleFrameStreaming();
              }, 1000); // Wait 1 second after video loads
            }
          };
        }
      } catch (error) {
    ...
    const handleVisibility = () => {
      if (document.hidden) {
        toast({
          ...
          description:
            "Tab switching detected. Please stay on the proctoring page.",
          variant: "destructive",
        });
      }
    };
    ...
    return () => {
      ...
      // Clean up frame streaming interval
      if (frameIntervalRef.current) {
        clearInterval(frameIntervalRef.current);
      }
    };
  }, [toast, isConnected, isStreamingFrames]);
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              ...
              <span style={{ fontWeight: "500" }}>Third Eye Activated</span>
            </div>
          </div>
        </div>
      )}
        ...
        >
          ...
          <div style={{ position: "relative" }}>
            ...
            >
              ...
              {/* Frame Streaming Toggle Button */}
              <button
                onClick={toggleFrameStreaming}
                disabled={!isConnected}
                style={{
                  position: "absolute",
                  bottom: "12px",
                  right: "12px",
                  backgroundColor: isStreamingFrames ? "#dc2626" : "#16a34a",
                  color: "white",
                  border: "none",
                  borderRadius: "6px",
                  padding: "8px 16px",
              ...
              >
                {isStreamingFrames ? "Stop AI Analysis" : "Start AI Analysis"}
              </button>
            </div>
          </div>
              ...
              >
                <div
                  style={{
                    width: "8px",
                    height: "8px",
                    backgroundColor: isStreamingFrames ? "#2563eb" : "#6b7280",
                    borderRadius: "50%",
                  }}
                />
                <span style={{ fontSize: "14px" }}>
                  AI Processing: {isStreamingFrames ? "Active" : "Inactive"}
                </span>
              </div>
            </div>
            <div
