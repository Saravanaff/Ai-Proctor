...
def detect_person_and_objects(image: np.ndarray) -> tuple[int, dict]:
    ...
    constants.detected_objects = {"Person": 0, "Mobile-phone": 0, "Laptop": 0}

    for box in result.boxes:
        label = yolo_model.names[int(box.cls[0])]
        if label == "person":
            constants.detected_objects["Person"] += 1
            constants.person_count += 1
        elif label == "cell phone":
            constants.detected_objects["Mobile-phone"] += 1
        elif label == "laptop":
            constants.detected_objects["Laptop"] += 1

    return constants.detected_objects
